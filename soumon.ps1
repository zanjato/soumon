...
&{
  ...
  function rqini{
    $my.serv=$serv
    $my.sou="http://s39websou$($my.serv).its.local"
...
public static class PSConIO{
  ...
  //public const uint MOUSE_MOVED=0x1;
...
'@
    ...
    $my.ir=[PSConIO+INPUT_RECORD[]](new-object PSConIO+INPUT_RECORD)
...
'@ -f $my.ars,$my.gu,$my.tu,$my.tok -replace '\r\n'
    rqdat $get
  }
...
'@ -f $my.ars,$inc,$my.tok -replace '\r\n'
  ...
  function cin{
    ...
    for(;;){
      ...
      $ior=[PSConIO]::readconsoleinput($my.cin,$my.ir,1,[ref]$cnt)
      ...
      if($ev.eventtype -eq [PSConIO]::KEY_EVENT){
        ...
        $ev=$ev.mouseevent
        if($ev.buttonstate -ne [PSConIO]::FROM_LEFT_1ST_BUTTON_PRESSED){continue}
        if($ev.controlkeystate -band $my.lck3){continue}
        if($ev.eventflags -ne [PSConIO]::DOUBLE_CLICK){continue}
